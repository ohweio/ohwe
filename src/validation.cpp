...
#if defined(NDEBUG)
# error "ohwecoin cannot be compiled without assertions."
#endif
...
const std::string strMessageMagic = "ohwecoin Signed Message:\n";
                              ...
                              bool fOverrideMempoolLimit, const CAmount& nAbsurdFee, std::vector<uint256>& vHashTxnToUncache)
{
...

    {
        ...
        {
            LogPrint("mempool", "replacing tx %s with %s for %s OHC additional fees, %d delta bytes\n",
...
CAmount GetBlockSubsidy(int nHeight, const Consensus::Params& consensusParams)
{
    ...
    //nSubsidy >>= halvings;
    //10000블록까지(2,805,000,000) 사전 채굴 10000블록 이후 보상 20
    if(nHeight < 10001) {
        nSubsidy = 280500 * COIN;
    }
    //500블록까지(2,805,000,000) 사전 채굴 10000블록 이후 보상 50
    /*
    if(nHeight < 500 ) {
        nSubsidy = 5610000 * COIN;
    }
    */
    return nSubsidy;
}
...
bool CVerifyDB::VerifyDB(const CChainParams& chainparams, CCoinsView *coinsview, int nCheckLevel, int nCheckDepth)
{
    ...
    if (nCheckDepth <= 0)
        // ohwecoin: suffices until year 10214. Didn't x4 value due to integer wrap around and upstream compatibility.
